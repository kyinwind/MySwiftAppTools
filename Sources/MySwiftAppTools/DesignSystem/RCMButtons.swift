...
// MARK: - 按钮样式
/// 自定义按钮样式，支持多种角色和全宽选项
struct RCMButtonStyle: ButtonStyle {
    let role: RCMButtonRole       // 按钮角色，决定颜色和边框
    let fullWidth: Bool           // 是否占满宽度
    let isDisabled: Bool          // 是否禁用状态

    init(role: RCMButtonRole = .secondary, fullWidth: Bool? = nil, isDisabled: Bool = false) {
        self.role = role
...
struct RCMSidebarIcon: View {
    let systemName: String  // SF Symbol 名称
    let tint: Color          // 背景颜色
    let size: IconSize       // 图标尺寸

    init(systemName: String, tint: Color, size: IconSize = .medium) {
...
extension RCMSidebarIcon {
    enum PresetTint {
        static let gray   = Color.gray
        static let blue   = RCMColor.accent      // 主题蓝色
        static let green  = RCMColor.success     // 成功绿色
        static let orange = RCMColor.warning     // 警告橙色
        static let red    = RCMColor.danger      // 危险红色
